...
grads = cell([iters_num 1]);

%train_loss_list = zeros(1, iters_num);
%train_acc_list = zeros(1, round(iters_num / iter_per_epoch));
%test_acc_list = zeros(1, round(iters_num / iter_per_epoch));
%acc_idx = 1;
...
%    b2 = network.params(1).b2;
    ...
    if mod(iter, iter_per_epoch) == 0
        ...
        %train_acc_list(1, acc_idx) = train_acc;
        %test_acc_list(1, acc_idx) = test_acc;
        %acc_idx = acc_idx + 1;
